      ...
      @if (products().length > 0) {
          ...
          @for (prod of products(); track prod.id; let i = $index) {
            <div
              ...
              <div class="w-full h-full rounded-3xl overflow-hidden border border-white/5 bg-white"
                ...
                <!-- Image -->
                <div class="relative h-[14rem] overflow-hidden bg-[#FDF5F0]">
                  <img
                ...
                <div class="p-4 h-[8rem] flex flex-col justify-between bg-white">
                  <div>
                    <span class="text-[8px] font-bold text-[#FF758F] uppercase tracking-wider block mb-0.5 font-serif italic">
                      {{ prod.nombre_japones }}
                    </span>
                    <h3 class="text-sm font-serif italic text-[#1A1A1A] font-bold leading-tight">
                      {{ prod.nombre_espanol }}
                    </h3>
                    <p class="text-[9px] text-[#1A1A1A]/50 mt-0.5 line-clamp-1">
                      {{ prod.descripcion_corta }}
                    </p>
                  </div>
                  <div class="flex items-center justify-between pt-1.5 border-t border-[#F0D5CC]">
                    @if (prod.precio_oferta) {
                      <span class="text-xs font-serif italic font-bold text-[#FF758F]">
                        {{ '$' + prod.precio_oferta.toLocaleString('es-CO') }}
                      </span>
                    } @else {
                      <span class="text-xs font-serif italic font-bold text-[#1A1A1A]">
                        {{ '$' + prod.precio.toLocaleString('es-CO') }}
                      </span>
                    }
                    <button
                      (click)="cartService.addToCart(prod, 1); $event.stopPropagation()"
                      class="w-7 h-7 rounded-full bg-[#FF758F] hover:bg-[#FF5277] text-white flex items-center justify-center transition-all active:scale-90"
        ...
        <div class="relative z-50 flex items-center justify-center gap-3 mt-4">
          <button (click)="prevSlide()"
            class="w-9 h-9 rounded-full bg-[#FFF0EA] border border-[#F0D5CC] flex items-center justify-center text-[#1A1A1A] hover:bg-[#FF758F] hover:text-white hover:border-transparent transition-all">
          ...
          @for (prod of products(); track prod.id; let i = $index) {
            <button (click)="goToSlide(i)"
              ...
              [style.background]="i === currentIndex() ? '#FF758F' : '#FFF0EA'"
              [style.color]="i === currentIndex() ? 'white' : '#1A1A1A'"
              [style.border]="i === currentIndex() ? 'none' : '1px solid #F0D5CC'">
              {{ i + 1 }}
            </button>
          }
...
export class ProductCarouselComponent implements OnDestroy {
  ...
  getShadow(index: number): string {
    const rel = Math.abs(this.getRelative(index));
    if (rel === 0) return '0 25px 60px -12px rgba(255,117,143,0.35)';
